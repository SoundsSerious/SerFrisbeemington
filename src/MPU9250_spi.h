...
public:
    ...
    unsigned int WriteReg_Fast(uint8_t WriteAddr, uint8_t WriteData );
    unsigned int WriteReg_SpiBus(uint8_t WriteAddr, uint8_t WriteData );
    unsigned int ReadReg(uint8_t WriteAddr, uint8_t WriteData );
    unsigned int ReadReg_Fast(uint8_t WriteAddr, uint8_t WriteData );
    void ReadRegs(uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes );
    void ReadRegs_Fast(uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes );
    ...
    void read_mag_interrupt();
    void read_mpu_interrupt();

    uint32_t cycle_count = 0;
    uint32_t max_cycles = 2000000000;

    void initialize();
    void update();
    void cycle();

    long backup_clock = 400000; //Conservative Clock For SPI
    long spibus_clock = 1000000; //Conservative Clock For SPI

    float acc_divider;
    float gyro_divider;

    uint8_t mag_read_count = 9;
    uint8_t mag_count;

    //Data Access Routines
    uint8_t mpu_interruptStatus, mag_interruptStatus;
    bool mpu_read, mag_read;
    uint8_t mag_overflowByte;
    bool mag_overflow;
